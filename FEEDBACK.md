...
- [x] Aumentar o tamanho do título do painel — **v0.3 (24/07/2026)**
- [x] Adicionar "(TZ)" no título, depois de "Tarifa Zero" — **v0.3**
- [x] ~~Outer glow bem sutil nos municípios ativos~~ — **descartado pelo autor (24/07/2026)**: preferência estética + risco de performance; na v0.3 os TZ ganharam contorno/stroke claro destacado (ver ROADMAP, registro de decisões)
- [x] **Borda dos estados (UF) sempre visível no mapa** — pedido 3x (Ivan, Carol, Ivan de novo) — **v0.3** (camada de UFs dissolvida dos municípios)
- [x] Deixar as bordas municipais mais visíveis no mapa em geral — **v0.3**
- [x] Destacar mais o título "TZ × Não-TZ — comparação (recorte atual)" (maior ou em negrito) — **v0.3**
- [x] Alinhar as caixas de resumo (cards) com os controles de filtro logo abaixo — **v0.3** (mesmo grid)
- [ ] Opção de minimizar/colapsar os painéis de gráficos (comparação TZ×Não-TZ e % por eixo) para ver o mapa junto com a tabela de municípios TZ sem rolar a página — *pendente (v0.4); glossário já é colapsável na v0.3*
- [x] Tooltip ao passar o mouse num município com TZ: mostrar as mesmas informações da linha dele na tabela — **v0.3**
- [x] Mais espaçamento entre o mapa e o painel lateral — **v0.3**
- [ ] Considerar cores/fontes de título diferentes por seção, para diferenciar visualmente os blocos (Gabriel) — *pendente (v0.4)*
- [x] Fundo branco (preferência do Gabriel) — **v0.3** como toggle claro/escuro, tema escuro continua o padrão
...
- [x] No bloco "% de municípios com Tarifa Zero por eixo": deixar claro que a % é dentro de cada categoria — **v0.3** (subtítulo)
- [ ] Diferenciar visualmente o bloco "% por eixo" do bloco "TZ × Não-TZ — comparação" (formas de apresentação distintas) — *pendente (v0.4); na v0.3 os dois blocos ganharam subtítulos explicando o que cada um compara, mitigação parcial*
- [x] Corrigir a associação visual barra↔legenda no bloco "% por eixo" — **v0.3** (rótulo acima da própria barra)
- [x] Auto-encaixe do mapa na seleção (UF filtrada → enquadra a UF; duplo clique → zoom local; botão "↺ Brasil") — **v0.3**; zoom/pan livre segue adiado (ver ROADMAP)
- [x] Ao selecionar uma UF no filtro, suprimir tooltip/hover dos municípios de fora do recorte — **v0.3**
- [x] Glossário de siglas (IBEU, IDH, PDMU, REGIC, NTU, arranjo metropolitano) — **v0.3** (bloco colapsável)

**Extras entregues na v0.3 (pedido da orientação):** cards de grandes números dinâmicos (reagem ao recorte, com aviso de amostra pequena; novo indicador "pessoas vivendo com TZ ativa" ≈ 7,6 mi) e linha do tempo das adoções/revogações por ano.
